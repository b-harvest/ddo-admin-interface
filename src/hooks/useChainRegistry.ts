...
const useChainRegistry = (chainName: COSMOS_CHAIN_NAME) => {
  ...
  const asset = useMemo(() => chainRegistry.findAssetsByChain(chainName), [])
  const chain = useMemo(() => chainRegistry.findChainByName(chainName), [])
  const IBC = useMemo(() => chainRegistry.findIBCByChain(chainName), [])
